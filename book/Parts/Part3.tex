...
\section{Linear Hyperbolic Problems}
...
\end{equation}
which can model, for instance, the tracing of a dye particle in a one-dimensional flow, such as a pipe. Suppose that you want to trace this particle. At time $t=0$, the particle is located at $x=0$ as shown in Figure~\ref{}. After some time, we expect the particle to have moved a distance $d=\alpha t$. Hence, it can be easily shown that solutions to Equation~\ref{eq:hyperprob_advection} are given by
\begin{equation}
	u(x,t) = u_0 (x - \alpha t, t), 
\end{equation}
where $u_0$ is the initial solution. Note that the particle has only translated, and no change in its shape or any diffusion effects are modelled by Equation~\ref{eq:hyperprob_advection}.

Consider now the space-time plane $x\times t$. In this space, we can find \textit{characteristic lines} $x(t)$ along which the solution remains unchanged, that is, lines where $du=0$. We can then rewrite the solution $u(x(t),t)$, and by using the chain rule 
\begin{equation}
...
\end{equation}
Hence the \textit{characteristic speed} of a scalar hyperbolic equation is the advection velocity. From the above relation, we obtain an equation for the characteristic lines of our PDE, given by
\begin{equation}
	x = x_0 + \alpha t,
\end{equation}
where a curve can be drawn for a given $x_0$. If we consider $\alpha$ to be positive, the resulting characteristic curves can be observed in Figure~\ref{fig:linscal_charact}. We can clearly note that advection components in the governing equations are responsible for the propagation of information. However, the most important conservation laws consist of more than a single equation. In order to analyze linear hyperbolic systems of equations, we need to reveal the underlying advection equations.
\begin{figure}[htbp]
	\centering
	\includegraphics[width=0.5\linewidth]{Pictures/ch11_linear_scalar_characteristics}
	\caption{Characteristic lines for the linear advection equation with $\alpha>0$, where $x_0$ are represented by dots.}
	\label{fig:linscal_charact}
\end{figure}
\subsection{Linear Hyperbolic Systems}
A system of $m$ equations
\begin{equation}
\frac{\partial \vec u}{\partial t} + A \frac{\partial \vec u}{\partial x} = 0,\quad \vec u = [u_1, u_2, \ldots, u_m]
\label{eq:hyperprob_linearsystem}
\end{equation}
is said to be hyperbolic if the constant $m\times m$ matrix $A$ has $m$ real distinct eigenvalues $\lambda_i$ and $m$ linearly independent eigenvectors $\vec q_i$. This can be written as the eigenvalue problem
\begin{equation}
...
\[
	...
	\end{bmatrix}.
\]
We have shown in the previous section that the scalar linear equation simply propagates information at a given characteristic speed, which represents the slope of the characteristic curves $x(t)$ in a space-time plane. In the case of a linear hyperbolic system, these slopes are given by the eigenvalues of $A$. Now, how are these characteristic speeds related to the conserved variables $\vec u$? In order to further analyze this, we need to transform the original system into its \textit{characteristic form}. The idea is to decouple Equation~\ref{eq:hyperprob_linearsystem} into a series of independent equations. 

Recall the $m$ eigenvectors of $A$ are linearly independent. Hence, we can diagonalize matrix A in Equation~\ref{eq:hyperprob_advection} by setting
\begin{equation}
	A = Q \Lambda Q^{-1}.
\end{equation}
Now, diagonalize $A$ and multiply Equation~\ref{eq:hyperprob_linearsystem} by $Q^{-1}$, which yields
\begin{equation}
	Q^{-1} \frac{\partial \vec  u}{\partial t} + Q^{-1} Q \Lambda Q^{-1} \frac{\partial \vec u}{\partial t} = 0.
\end{equation}
For simplicity, we define a \textit{characteristic variable} $\vec \omega=Q^{-1} \vec{u}$ and hence we obtain the characteristic form of the system of PDEs
\begin{equation}
	\frac{\partial \vec \omega}{\partial t} + \Lambda \frac{\partial \vec \omega}{\partial x} = 0.
\end{equation}
Hence, We have decoupled our original system into $m$ independent advection equations of the form
\begin{equation}
	\frac{\partial \omega_i}{\partial t} + \lambda_i \frac{\partial \omega_i}{\partial x} = 0,\quad i=1,\ldots,m,
\end{equation}
each of which has solution
\begin{equation}
	\omega_i = \omega_{i,0}(x-\lambda_i t).
\end{equation}
Similar to the scalar case, we can now draw characteristic curves in the space-time plane. If we consider the eigenvalues to be sorted such that
\begin{equation}
	\lambda_1 < \lambda_2 < \ldots < \lambda_m,
\end{equation}
then the result can be seen in Figure~\ref{}. What we have done is an eigendecomposition of the vector of conserved quantities $\vec u$ into characteristic variables $\vec \omega$ by means of the eigenvectors $\vec q_i$. Hence, the original variables can be recovered by a superposition of the characteristic waves
\begin{equation}
	u(x,t) = \sum_{i=1}^{m} \omega_i(x,t) \vec q_i.
\end{equation}
