...
Now, consider the problem where the initial conditions are given by
\begin{equation}
	u(x,0) = 
	\begin{cases}
		0 & \text{if}~x < 0,\\
...
As we observed in the second problem, weak solutions need not be unique, and hence we must add a constraint to single out those relevant in the physical world.  Hence, to identify physical solutions, we consider a shock wave to be admissible only if the following entropy condition is satisfied 
\begin{equation}
	f'(u_L) > s > f'(u_R),
\end{equation}
...
Applications of engineering interest are typically described by a system of conservation laws of the form
\begin{equation}
	\frac{\partial \vec u}{\partial t} + \frac{\partial \vec F (\vec u)}{\partial x} = 0,
\end{equation}
where $\vec u$ is the vector of conserved variables and $\vec F(\vec u)$ is the vector of fluxes. In Section~\ref{sec:linear_hyper_systems}, we stated that a system is said to be hyperbolic if the flux Jacobian matrix has real eigenvalues and a complete set of linearly independent eigenvectors. Previously, we considered constant Jacobian matrices that led to a set of linear equations. In this section, we consider problems whose Jacobian matrices have the form 
\begin{equation}
	A(\vec u) = \frac{\partial \vec F(\vec u)}{\partial \vec u},
\end{equation}
and hence both the eigenvalues and eigenvectors depend on the value of the solution, which in turn depends on time. An example of a nonlinear hyperbolic system is given by the Euler equations, which can be written in one dimension
\begin{equation}
...
\begin{equation}
	...
	\end{bmatrix}.
\end{equation}
$\rho$ is density, $v$ is velocity and $E$ is the specific energy. Recall that the speed of sound is given by
\begin{equation}
...
\begin{equation}
	...
	\end{bmatrix},
\end{equation}
To calculate the Jacobian of the flux vector, we write the Euler fluxes in terms of the conserved variables $u_1,~u_2,~u_3$
\begin{equation}
	\vec F = 
	\begin{bmatrix}
		f_1 \\ f_2 \\ f_3
	...
	\end{bmatrix}.
\end{equation}
This yields the Jacobian matrix for the Euler equations
\begin{align}
	A(\vec u) = 
	\begin{bmatrix}
		0 & 1 & 0 \\
...
\begin{eqBox}
	...
	\end{align}
\end{eqBox}
We have stated in previous sections that hyperbolic equations can be thought of as a superposition of travelling waves, with wavespeeds the eigenvectors of the Jacobian matrix. The eigenvalues of $A(\vec u)$ are 
\begin{equation}
	\lambda_1 = v - c, \quad\quad \lambda_2 = v, \quad\quad \lambda_3 = v + c,
\end{equation}
which shows that the Euler equations can be thought of a superposition of a left-travelling sound wave, a momentum wave and a right-going sound wave, consistent with the propagation of sound. The corresponding eigenvectors are 
\begin{equation}
...
\begin{align}
	...
	\end{bmatrix}.
\end{align}
After $t=0.2$, the exact solution can be seen in Figure~\ref{fig:shocktubesol}. The figures are divided into five regions, which we call A, B, C, D, E, from left to right. 
\begin{figure}[htbp]
	\centering
...
\begin{itemize}
	...
	\item At the interface between regions C and D, we observe a contact discontinuity. This wave represents the effects of the initial separation of the left and right states travelling to the region of low pressure. Across this wave, pressure and velocity remain constant, but density and quantities that depend on it contain jumps.
...
\begin{align}
	...
	\end{cases}
\end{align}
Roe proposed considering instead a constant Jacobian matrix $\tilde A$ with the following properties
\begin{itemize}
	...
	\item $\tilde A$ must be consistent with the exact problem $\tilde A (\vec u, \vec u) = A(\vec u)$.
...
\begin{equation}
	\vec F_{i+1/2} = \vec F(\vec u_{i+1/2}).
\end{equation}
At the interface between $\Omega_i$ and $\Omega_{i+1}$ defined at $x_{i+1/2}$, the value of the flux is generally obstained from a constant vector of the solution within the cell, i.e. $\vec u_{i+1/2}=\vec u_{i}$. This approach is known as the Godunov method, which is first order accurate. Nevertheless, we can derive methods with orders of accuracy higher than one, allowing the numerical error to significantly decrease. Increasing the order of a finite-volume method can be done replacing the constant interface values of the solution $\vec u_{i+1/2}$ by a reconstructed value interpolated introducing information from neighbouring cells. This idea, developed by van Leer, is an extension of Godunov's method, and is known as the MUSCL (monotone upstream-centered schemes for conservation laws) reconstruction approach. 

For a second order scheme, we consider the solution to be a linear Taylor series expansion around the cell center $x_j$ by
\begin{equation}
	u(x) = u_i + (x-x_j)\frac{\partial u_i}{\partial x},
	\label{eq:3209423}
\end{equation}
where $\frac{\partial u}{\partial x}$ can be computed using cell solution differences. Note that this reconstruction process can be done using additional terms in Equation~\ref{eq:3209423} to yield higher order schemes. Hence, on the left and right sides of the interface at $x=x_{i+1/2}$, the solution can be found by
\begin{align}
	\vec u_{i+1/2}^L &= \vec u_i + \frac{1}{2} \vec \delta_i,\\
	\vec u_{i+1/2}^R &= \vec u_{i+1} - \frac{1}{2} \vec \delta_{i+1},
\end{align}
where the so-called slopes $\delta_i$ can be defined using a blended approach
\begin{equation}
	\vec \delta_i = \frac{1}{2}(1+r)\delta \vec u_{i-1/2} + \frac{1}{2}(1-r)\delta \vec u_{i+1/2},
	\label{eq:48742938}
\end{equation}
such that $r=1$ is a upwind-biased approximation of the derivative. In Equation~\ref{eq:48742938},
\begin{align}
	 \delta \vec u_{i+1/2} = \vec u_{i+1} - \vec u_i, \\
	 \delta \vec u_{i-1/2} = \vec u_{i} - \vec u_{i-1}.
\end{align}
Then, the common flux can be computed using an appropriate Riemann solver
\begin{equation}
	\vec F_{i+1/2} = F(\vec u^L_{i+1/2},\vec  u^R_{i+1/2}).
\end{equation}
\subsection{Second-Order Upwind Scheme for Linear Advection}
For linear advection, we have shown that an appropriate solver is the upwind Riemann flux, given by
\begin{equation}
...
\end{align}
where we choose upwind-biased slopes in Equation~\ref{eq:48742938} with $r=1$ such that
\begin{align}
...
We can measure the total variation (TV) of a discrete solution
\begin{equation}
...
\begin{equation}
	TV(u^t+1)\leq TV(u^t).
\end{equation}
In addition, Godunov's theorem states
\begin{theorem}
...
\end{theorem}
Hence, while higher-order schemes are more accurate for smooth solutions, they introduce spurious oscillations in the presence of discontinuities, shocks and large solution gradients and are thus not TVD. Different approaches can be used to ensure the TVD property for high-order numerical methods. In the next section, we discuss one of these techniques, known as flux/slope limiters.
...
Consider rewriting the MUSCL formulation in Equation~\ref{eq:48742938} using limited slopes 
\begin{align}
	\overline \delta_i = \frac{1}{2}(1+r)\phi^+_{i-1/2}\delta u_{i-1/2} + \frac{1}{2}(1-r)\phi^-_{i+1/2}\delta u_{i+1/2},
\end{align}
...
\begin{eqnarray}
	\phi^{\pm}_{i+m} = \phi(r^{\pm}_{i+m}),\quad r^{\pm}_{i+m} = \frac{\delta u_{i+m\pm 1}}{\delta u_{i+m}}.
\end{eqnarray}
Consider the second-order upwind-biased scheme in Equation~\ref{eq:34290}. We can rewrite it using slope limiters using interface fluxes of the form
\begin{align}
	F_{i+1/2} &= F(u_{i+1/2}) = \alpha u_i + \frac{\alpha}{2}\phi^+_{i-1/2}\left(u_i-u_{i-1}\right),\\
	F_{i-1/2} &= F(u_{i-1/2}) = \alpha u_{i-1} + \frac{\alpha}{2}\phi^+_{i-3/2}\left(u_{i-1}-u_{i-2}\right).
\end{align}
Clearly, in the case $\phi(r) = 0$, the scheme will reduce to the first-order upwind scheme. Some slope limiting functions are shown in Table~\ref{table:limiters}. We now show an application of these functions using the linear advection equation.
\begin{table}[h]
	\centering
	\caption{Common forms of slope limiter functions $\phi(r)$}
	\begin{tabular}{cl}
		\hline
		Name & $\phi(r)$ \\ \hline
...
\end{align}
on a grid with $x\in[0,2]$ with periodic boundary conditions. The solution at $t=0$ is given by
\begin{align}
	u(x, 0) &= 
	\begin{cases}
	...
	\end{cases}
\end{align}
As we see in Figure~\ref{fig:}, the initial conditions consist of a smooth gaussian profile and a step function. Due to the nature of the equation, we only expect a translation of the initial condition through the domain. At $t=2$, a complete advective cycle has passed and $u(x,2)$ is expected to be equal to $u(x,0)$. Using the second-order advection scheme with $r=1$ and $N=300$ cells, the solution is shown in Figure~\ref{fig: }. In the smooth region of the domain, the second-order scheme does a good job representing the solution, but we observe that the discontinuities on the right side of the domain contain large oscillations that overshoot and undershoot the solution. 

Implementation of the limiters in Table~\ref{table:limiters} removes the presence of oscillations in the solution. 
...
\begin{align}
	...
	\end{bmatrix}.
\end{align}
on a domain $x\in[0, 1]$ and $t\in(0,0.2]$. Figure~\ref{} shows a comparison between Godunov's scheme and a linear MUSCL-reconstructed method with different limiters. The limited slopes for this problem have the form 
\begin{equation}
	\vec {\overline \delta_i} = \frac{1}{2}(1+r)\vec \phi^+_{i-1/2}
	\begin{bmatrix}
		\delta \rho_{i-1/2} \\ \delta (\rho u)_{i-1/2} \\ \delta (\rho E)_{i-1/2}
	\end{bmatrix} + \frac{1}{2}(1-r)\vec \phi^-_{i+1/2}
	\begin{bmatrix}
		\delta \rho_{i+1/2} \\ \delta (\rho u)_{i+1/2} \\ \delta (\rho E)_{i+1/2}
	\end{bmatrix}
\end{equation}
...
\end{figure}
\begin{exercise}
	Compute the total variation (TV) of the above example using the minmod, van Leer and superbee limiter functions. Which one is the most suitable for this problem?
\end{exercise}
\begin{exercise}
	Repeat the previous problem with initial conditions
	\begin{itemize}
		\item \begin{align}
			u^1(x, 0) &= 
			\begin{cases}
...
\begin{jupyternote}
	Check out the High-Order Finite Volume Jupyter notebook \href{\binderurl}{\underline{here}}. You can also download the files from the Gitlab repository \href{\repourl}{\underline{here}}.
\end{jupyternote}
