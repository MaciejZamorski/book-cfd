...
\begin{align}
	...
	u_{i+1/2}^R &= u_{i+1}.
\end{align}
For a better resolution of smooth solutions, we can derive methods with orders of accuracy higher than one, allowing the numerical error to significantly decrease. This can be done by replacing the constant interface values of the solution $\vec u_{i+1/2}$ for a reconstructed value, which is interpolated using information from neighbouring cells. This idea, developed by van Leer, is an extension of Godunov's method, and is known as the MUSCL (monotone upstream-centered schemes for conservation laws) reconstruction approach. A graphical representation of the method is shown in Figure~\ref{fig:muscl_scheme}.
\begin{figure}[htbp]
	\centering
...
\begin{equation}
	\vec \delta_i = \frac{1}{2}(1+b)\delta \vec u_{i-1/2} + \frac{1}{2}(1-r)\delta \vec u_{i+1/2},
	\label{eq:48742938}
\end{equation}
such that $r=1$ results in an upwind-biased approximation. Considering a linear reconstruction, the solution slopes are given by
\begin{align}
...
For linear advection, acommon choice is the upwind Riemann flux given by
\begin{equation}
...
The total variation (TV) of a scalar conservation law such as the linear advection equation is given by 
\begin{equation}
...
\begin{equation}
	TV(u^t+1)\leq TV(u^t).
\end{equation}
Harten proved that TVD schemes are monotonicity-preserving, meaning that in time,maxima does not increase, minima does not decrease, and no new extrema are generated in the solution. It turns out that this property can only be attributed to certain types of schemes. Godunov's theorem states
\begin{theorem}
...
\end{align}
Clearly, in the case $\phi(r) = 0$, the flux reduces to the first-order upwind scheme. Some slope limiting functions are shown in Table~\ref{table:limiters}. In the next section, we show applications of these limiters in the context of linear advection and the Euler equations.
\begin{table}[tb]
	\centering
...
A comparison of the TVD properties of the scheme and other additional assessments can be made via the MUSCL Schemes Jupyter notebook. 
\begin{jupyternote}
...
The analysis of these results is similar to the advection example and is left as an exercise to the student. The corresponding Jupyter notebook shows the implementation of these functions for the Euler equations.

\begin{figure}[htb]
	\centering
	\includegraphics[width=\linewidth]{Pictures/euler_muscl}
	\caption{Results of Sod's shock tube problem at $t=2$ with initial conditions in Equation~\ref{eq:39248243}}
