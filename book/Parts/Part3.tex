...
To get around this problem we can use {\it Newton-Raphson} iterations. First assume that the Jacobian matrix can be {\it approximated} using the current known solution as
\begin{equation}
	A_0 = 
	\begin{bmatrix}
...
\end{equation}
and the entire procedure is then repeated for each subsequent time step. Hence, for non-linear equations we need to solve multiple linear systems for each time step, following Newton-Raphson, which can significantly increase their computational cost relative to linear systems. Furthermore, convergence of Newton-Raphson is not guaranteed. Hence, non-linear systems often require smaller time steps relative to linear systems to maintain stability of the iterative method.
