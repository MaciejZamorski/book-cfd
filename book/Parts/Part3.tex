...
\begin{equation}
	...
	\end{bmatrix}
\end{equation}
The eigenvalues of the Euler system are distinct and that a complete set of eigenvectors can be found, hence the Euler equations are strictly hyperbolic. 
\subsection{The Riemann Problem}
...
\end{equation}
Solving the exact Riemann problem for the Euler equations turns out to be very expensive. In the case of linear equations, the Jacobian matrix is constant. However, as shown in the previous section, this matrix depends on the value of the solution. Hence, computing the wave speeds introduces a large computational cost. In addition, we typically require the solution of nonlinear systems to compute the Riemann pressure. For more details on the exact Riemann solution for the Euler equations, consult~\cite{levequeFiniteVolumeMethods2002}. 
\subsection{An approximate Riemann solver for the Euler Equations}
...
\begin{equation}
	\frac{\partial \vec u}{\partial t} + A(\vec u) \frac{\partial \vec u}{\partial x} = 0.
\end{equation}
For the Euler equations, $A(\vec u)$ is given by Equation~\ref{eq:eulerjacobian}. Note that the eigenvalues of $A$ depend on the value of the solution and hence vary in time. We are interested in finding solutions at the interfaces between cells, where the problem can be written
\begin{equation}
	\frac{\partial \vec u}{\partial t} + \frac{\partial \vec f}{\partial x} = 0,
\end{equation}
...
\begin{itemize}
	\item Hyperbolic, according to Equation~\ref{},
...
\end{itemize}
Now, consider a numerical scheme of the Godunov form
\begin{equation}
	\vec u^{t+1}_i = \vec u^t_i - \frac{\Delta t}{\Delta x} \left(\vec f_{i+1/2} - \vec f_{i-1/2}\right) 
\end{equation}
where $f_{i+1/2}$ is a numerical flux at the interface between cell $\Omega_i$ and $\Omega_{i+1}$ and has the form
\begin{equation}
	\vec f_{i+1/2} = \vec f (\vec u_{i+1/2}),
\end{equation}
To approximate the flux at the interface, we need to compute the intermediate states in the star region.
