...
\begin{figure}[htbp]
	...
	\label{fig:fv_scheme}
\end{figure}
In the previous chapter, we derived and characterized the finite difference method. While it is generally simple to apply, it requires the use of structured grids. Suppose now you want to simulate flow over an aircraft or one of its components. Generating structured meshes can become challenging, if not impossible, for complex geometries. Hence, we need methods that are flexible enough such that the geometry is not a fundamental limitation. Consider subdividing a general domain $\Omega$ into volumes or cells $\Omega_i$, as shown in Figure~\ref{fig:fv_scheme}. Within each cell, the value of the solution can be assumed to be constant if the size of $\Omega_i$ is sufficiently small. This idea leads to derivation of the Finite Volume method, which is one of the most commonly used schemes in the aerospace industry for CFD simulations. It was originally developed for its application on integral conservation laws. In this chapter, we derive the FV method and discuss the implication of solving the conservation laws using an integral approach.  
...
\begin{equation}
    ...
    \label{eq:fv_gcl}
\end{equation}
where $\vec{u}$ is a vector of conserved variables, such as mass, momentum, and energy. $\vec{F}=\vec F(\vec f)$ is the vector of fluxes, $S$ is the area of $\Omega$ with outward unit normal $\hat n$. Recall that for constant volumes, integration and differentiation commute according to Leibniz's rule, so we can rewrite Equation~\ref{eq:fv_gcl} with the time derivative outside of the integral in the first term
\begin{equation}
    \frac{\partial}{\partial t} \int_\Omega \vec{u} d\Omega + \int_s \vec F\cdot \hat n ds = 0.
    \label{eq:fv_gclleibniz}
\end{equation}
Clearly, the integral of the solution yields its total amount within $\Omega$. Hence, the rate of change of the total amount of $\vec{u}$ in a volume is the result of the net flux accross the surface $S$ of $\Omega$. We are looking for an average value of the solution, specifically the \textit{cell-average} of $\vec u$, which can be written
\begin{equation}
    \vec {\overline{u}} = \frac{ \int_\Omega \vec u d\Omega} {|\Omega|},
    \label{eq:fv_average}
\end{equation}
where $|\Omega|$ is the size of the cell. After averaging the solution, it is clear that it has now become constant within each volume, i.e. $\vec {\overline u} \neq \vec {\overline u} (\vec x)$, so it is not only a function of time $\overline {\vec u} = \overline {\vec u} (t)$. If we combine Equations~\ref{eq:fv_gclleibniz} and~\ref{eq:fv_average}, we obtain
\begin{equation}
    \frac{d \vec{\overline u}}{dt} + \frac{1}{|\Omega|} \int_s \vec{F} \cdot\hat nds=0.
\end{equation}
Now, we have an equation for the rate of change of the averaged quantity $\vec{\overline u}$. What remains is to define the value of $\vec{F}$. First, we note that the averaged solution must converge to the instantaneous solution in the limit of a sufficiently small volume
\begin{align}
    \lim_{|\Omega| \rightarrow 0} \vec{\overline u} = \vec u,
\end{align}
then, the value of its corresponding flux can be approximated by
\begin{equation}
...
Generally, we can make relatively good approximations of arbitrary volumes using straight-sided elements such as triangles and tetrahedra. Consider the triangular volume $\Omega_j$ displayed in Figure~\ref{fig:fv_straightsided}. The solution $\vec{\overline u}$ is constant within the triangular shape. In addition, each of the faces has an area of $S_1$, $S_2$ and $S_3$. Observe that since the sides of the considered volume are straight, the flux vectors are constant accross each of the faces. Hence, the surface integral in Equation~\ref{eq:fvm} can be trivially solved via summation. For the $j$-th volume, a the general form of the FV method for straight grids can be written
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.4\linewidth]{Pictures/ch11_fv_cellavg}
    \caption{Representation of solution, fluxes and faces of a straight-sided element.}
    \label{fig:fv_straightsided}
\end{figure}
\begin{eqBox}
    \begin{equation}
        \frac{d \overline{\vec u}_j}{dt} = - \frac{1}{|\Omega_j|} \sum_{i=1}^{m} \vec F_i \cdot \hat n_i S_i,
    \end{equation}
\end{eqBox}
where $m$ is the number of faces, $F_i$ is the flux across the $i$-th face with outward unit normal $\hat n_i$ and area surface and $S_i$.
